...
locationsArray.forEach((location) => {
   ...
   searchByLocation.addEventListener('change', () => {
    searchByParkType.options[searchByParkType.selectedIndex].textContent = 'Search By ParkType';
...
searchByParkType.addEventListener('change', () => {
   searchByLocation.options[searchByLocation.selectedIndex].textContent = 'Search By Location'; 
   const currentSelectedPark = searchByParkType.value;
   const locationName = nationalParksArray.LocationName;
   tbody.innerHTML= '';
   const selectedParks = nationalParksArray.filter((nationalPark) => nationalPark.LocationName.indexOf(currentSelectedPark) !== -1)
